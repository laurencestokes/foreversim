...
func talentLadder() SpellDataTable {
	return SpellDataTable{
		{Rank: 1, SpellID: 20468, Effects: []SpellDataEffect{
			{Index: 0, Effect: E_APPLY_AURA, Aura: A_ADD_PCT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_ALL_EFFECTS), Value: 16},
			{Index: 1, Effect: E_APPLY_AURA, Aura: A_ADD_FLAT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_EFFECT2), Value: -2}}},
		{Rank: 2, SpellID: 20469, Effects: []SpellDataEffect{
			{Index: 0, Effect: E_APPLY_AURA, Aura: A_ADD_PCT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_ALL_EFFECTS), Value: 33},
			{Index: 1, Effect: E_APPLY_AURA, Aura: A_ADD_FLAT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_EFFECT2), Value: -4}}},
		{Rank: 3, SpellID: 20470, Effects: []SpellDataEffect{
			{Index: 0, Effect: E_APPLY_AURA, Aura: A_ADD_PCT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_ALL_EFFECTS), Value: 50},
			{Index: 1, Effect: E_APPLY_AURA, Aura: A_ADD_FLAT_MODIFIER, Misc: int32(dbcenums.SPELLMOD_EFFECT2), Value: -6}}},
	}
}
...
func TestLadderUntakenTalentIsZero(t *testing.T) {
	table := talentLadder()
	if got := table.Effect(A_ADD_PCT_MODIFIER, int32(dbcenums.SPELLMOD_ALL_EFFECTS)).ValueAt(0); got != 0 {
		t.Errorf("rank 0 value: want 0, got %v", got)
	}
	if got := table.Effect(A_ADD_FLAT_MODIFIER, int32(dbcenums.SPELLMOD_EFFECT2)).MultiplierAt(0); got != 1 {
...
func TestLadderIsNotPerPointTimesRank(t *testing.T) {
	threat := talentLadder().Effect(A_ADD_PCT_MODIFIER, int32(dbcenums.SPELLMOD_ALL_EFFECTS))
...
func TestLadderMultiplierTakesItsSignFromTheData(t *testing.T) {
	if got := talentLadder().Effect(A_ADD_FLAT_MODIFIER, int32(dbcenums.SPELLMOD_EFFECT2)).MultiplierAt(3); math.Abs(got-0.94) > 1e-9 {
