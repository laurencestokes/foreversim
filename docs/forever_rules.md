...
Read from the client, build 1.60.1.70009: each race's racial skill line in `SkillLineAbility`, then
...
| Rule | Source | Here |
|---|---|---|
| Every +10 resistance racial is removed. Blood Elf and Draenei are not Forever races: no class can pick them, and Draenei is gone from the race list. | Client: `CharBaseInfo`, racial skill lines | `sim/core/character_constants.go` |
| Race and class pairings follow the client: dwarf shaman, undead paladin, orc mage, troll warlock, gnome priest and human hunter are new; Skyborne are warrior, hunter, rogue or druid on either side, mage only as High Order, shaman only as Windshaper. Skyborne base attributes are the Human's with +1 Agility, -1 Strength, +1 Intellect and -1 Stamina, read from level 1 character sheets (wowsims). | Client: `CharBaseInfo`; character sheets | `sim/core/character_constants.go`, `base_stats.go` |
...
| Orc: Blood Fury +10% attack power, ranged attack power and spell power (damage and healing) for 15 s, 2 min (20572). Command is gone; Shatter Curse is -15% magic damage taken for 8 s, 3 min (1299026). | Client | |
| Night Elf: Quickness +1% dodge and +2% run speed (20582). Elune's Light +10% crit for 15 s, 3 min (1259799). | Client | |
| Gnome: Expansive Mind +5% maximum rage, energy or mana by class (1259802/1259803/20591), not Intellect. Eureka! (one spell per class): the next 3 damaging abilities within 15 s cost 10% less and deal 10% more, 2 min. Build 70009 set the cost cut to 10% for every class (69977 had warrior 40%, rogue 20%, mage 50%, warlock 50%, priest 15%). Wired for the warrior, warlock, rogue, mage and priest ability lists (`Character.EurekaSpellMask`, set in `sim/warrior/warrior.go`, `sim/warlock/warlock.go`, `sim/rogue/rogue.go`, `sim/mage/mage.go` and `sim/priest/priest.go`; `EurekaChargeMask` when an ability's hits are spells of their own, as Mutilate's are, or when a channel's periodic damage is cast as its own spell reusing the channel's mask, as Blizzard's is). The rogue list leaves out Blade Flurry, which is on the client mask but deals no damage itself, per the tooltip's "damaging abilities"; the priest list leaves out the client mask's heals for the same reason, and Mana Burn because the sim does not model it. | Client | `registerEureka` |
